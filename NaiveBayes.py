...
if st.session_state.get("model") is not None:
    ...
    if st.button("Vypočítať posterior"):
        ...
        st.write(posterior)
        try:
            prior = inference.query(variables=[target_col])
            ...
            comp_df = pd.DataFrame(
                {
                    ...
                    "Východisková pravdepodobnosť": prior_probs,
                    "Aktualizovaná pravdepodobnosť": post_probs_aligned,
                }
            )
            st.subheader("Porovnanie: východisková vs. aktualizovaná pravdepodobnosť")
            st.table(comp_df)

            #stlpcový graf: prior vs posterior
            fig, ax = plt.subplots()
            x = list(range(len(prior_states)))
            width = 0.35

            ax.bar([i - width / 2 for i in x], prior_probs, width, label="Východisková (prior)")
            ax.bar([i + width / 2 for i in x], post_probs_aligned, width, label="Aktualizovaná (posterior)")

            ax.set_xticks(x)
            ax.set_xticklabels(prior_states)
            ax.set_ylabel("Pravdepodobnosť")
            ax.set_title(f"Porovnanie rozdelení pre {target_col}")
            ax.legend()

            st.pyplot(fig)

        except Exception as e:
            st.warning(f"Nepodarilo sa zobraziť porovnávaciu vizualizáciu: {e}")